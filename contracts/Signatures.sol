...
library Signatures {
    ...
    /// @notice Non-reverting version of ECDSA.recover that returns address(0) if anything is invalid
    ...
    /// @notice Similar to `return (_fullSignature[:65], _fullSignature[65:]);` with `bytes calldata` but for `bytes memory`
